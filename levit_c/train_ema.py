...
def train(args):
    ...
    eval_metrics = {'Top_1_Accuracy': nn.Top1CategoricalAccuracy(),
                    'Top_5_Accuracy': nn.Top1CategoricalAccuracy()}
    evel_name = list(eval_metrics.keys())
    eval_indexes = [0, 1, 2]
    # init model
    if args.use_ema:
        ...
        if args.dynamic_loss_scale:
            logger.info(f"Using DynamicLossScaleUpdateCell")
            loss_scale_manager = nn.DynamicLossScaleUpdateCell(loss_scale_value=2 ** 24, scale_factor=2,
                                                               scale_window=2000)
        elif args.dynamic_loss_scale is False and args.loss_scale >1:
            logger.info(f"Using FixedLossScaleUpdateCell, loss_scale_value:{args.loss_scale}")
            loss_scale_manager = nn.FixedLossScaleUpdateCell(loss_scale_value=args.loss_scale)
        else:
            logger.info(f"loss_scale is not used")
            loss_scale_manager = Tensor(args.loss_scale, ms.float32)
        ...
        model = Model(net_with_loss, eval_network=eval_network, metrics=eval_metrics, eval_indexes=eval_indexes)
    else:
